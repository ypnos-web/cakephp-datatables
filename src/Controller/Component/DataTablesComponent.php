...
class DataTablesComponent extends Component
{
    ...
    private function _order(array &$options, array &$columns)
    {
        ...
        foreach ($this->request->query['order'] as $item) {
            ...
            $c = isset($columns[$item['column']]) ? $columns[$item['column']] : null;
            if (!$c || (isset($c['orderable']) && !$c['orderable'])) // orderable is true by default
     ...
     */
    private function _filter(array &$options, array &$columns)
    {
        ...
        if (!empty($this->request->query['search']['value'])) {
            $globalSearch = $this->request->query['search']['value'];
            if (empty($columns))
            ...
            $options['globalSearch'] = $globalSearch;
                ...
                foreach ($columns as $c) {
                    if (isset($c['searchable']) && !$c['searchable']) // searchable is true by default
                        continue;
        ...
        foreach ($this->request->query['columns'] as $index => $column) {
            if (!empty($column['search']['value'])) {
                ...
                $c = (isset($columns[$index]) ? $columns[$index] : null);
                if (!$c || (isset($c['searchable']) && !$c['searchable'])) // searchable is true by default
     ...
     */
    public function find($tableName, $finder = 'all', array $options = [], array $columns = [])
    {
        $delegateSearch = !empty($options['delegateSearch']);
